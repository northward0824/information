...
const lishishangbaohechabanjiedetials = params => {
	return fetch({
		url: `/ifcs-api/api/event/check/getCheckDetail/` + params.eventId,
		method: "get",
	});
};
//案件核查提交 
...
const zhuanxiangcaijidiandetails = params => {
	return fetch({
		url: `/ifcs-api/api/special/getDetailSpecialTask/` + window.localStorage.getItem("zhuanxiangcaijiyiwanchengid") +
			'/' + window.localStorage.getItem("caijidianisdsid"),
		method: "get",
		params: params
	});
};
...
const process = params => {
	return jsonAxios.jsonAxios.post("/ifcs-api/api/event/verifyTask/process/" + params.taskId, params);
};
///event/getProcessDetail/{eventId}/{processId}
const getProcessDetail = params => {
	return fetch({
		url: `/ifcs-api/api/event/getProcessDetail/${params.eventId}/${params.processId}`,
		method: "get",
	});
};
