<template>
	<div>
		<van-nav-bar title="案件核查" left-text="返回" left-arrow @click-left="onClickLeft" @click-right="onClickRight">
		...
		<div style="overflow: auto;position: absolute;left: 0;right: 0;bottom: 0;top: 2.2rem;">
			<van-tabs color="#6A7CFC" @click="onClick">
				...
				<van-tab title="已完成">
					<div class="list-con" v-for="item in list">
						<ul @click="yiwancheng(item.eventId,item.eventProcessId)">
							<li style="margin-bottom:0">
								<h3>{{item.eventCode}}</h3>
								<div class="righslskos">
									<img src="../../assets/dingwei.png" alt />
...
<script>
	export default {
		...
		methods: {
			...
			yiwancheng(id,ids) {
				window.localStorage.setItem("anjianhechaderailsUid", id);
				window.localStorage.setItem("processId", ids);
