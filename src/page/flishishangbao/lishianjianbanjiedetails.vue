      ...
      <div class="zhongjian">
        ...
        <div>
          <div style="float:left;text-align: center;">
            ...
            <div>
              <img style="width:2.5rem;height:3rem;" :src="sldkjfwiosssss.imageDetailUrl" alt />
            </div>
          </div>
          <div style="float:right;text-align: center;">
            <div style="color:gray;margin:.1rem 0rem;">远景图(核实)</div>
            <div>
              <img style="width:2.5rem;height:3rem;" :src="sldkjfwiosssss.imagePanoramaUrl" alt />
            </div>
          </div>
          <div style="clear:both;"></div>
        </div>

         <div style="border-bottom:1px solid #F2F2F2;padding-bottom:.1rem;margin-bottom:.1rem;margin-top:.4rem;">
          <div style="float:left;color:gray;">是否属实 :</div>
          <div style="float:right;width:75%;" v-if="sldkjfwiosssss.isFinished == 0">否</div>
          <div style="float:right;width:75%;" v-if="sldkjfwiosssss.isFinished == 1">是</div>
          <div style="clear:both;"></div>
        </div>

         <div style="border-bottom:1px solid #F2F2F2;padding-bottom:.1rem;margin-bottom:.1rem;">
          <div style="float:left;color:gray;">核查情况 :</div>
          <div style="float:right;width:75%;">{{sldkjfwiosssss.detail}}</div>
          <div style="clear:both;"></div>
        </div>

         <div style="border-bottom:1px solid #F2F2F2;padding-bottom:.1rem;margin-bottom:.1rem;">
          <div style="float:left;color:gray;">位置信息 :</div>
          <div style="float:right;width:75%;">{{sldkjfwiosssss.location}}</div>
          <div style="clear:both;"></div>
        </div>
     ...
     getsldkfjpaidosjgs(){
      ...
      this.$api
        ...
        .then(res => {
          if (res) {
            if (res.data.code == 0) {
              console.log(res.data.data.apiEventProcessDTO);
              this.sldkjfwiosssss = res.data.data.apiEventProcessDTO;
            }
          }
        })
        .catch(err => {
